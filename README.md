...
1. DOM nedir?
   DOM sayfamızın yapısındaki nesneleri ve bu nesnelerin alt nesnelerini içeren modeldir.
2. Bir event nedir?
   Event herhangi bir işlem olabilir tıklama kaydırma gibi.
3. Bir event dinleyici(listener) nedir?
   gerçekleşen eventin neticesinde ne olacağını belirttiğimiz fonksiyon.
4. Neden bir NodeList'i Array'e dönüştürelim??
   nodelistte arrayde kullandığımız metodların hepsini kullanamıyoruz.
5. Bileşen nedir?
   dom öğesi oluşturmamıza yardım eden fonksiyon.
